...
from django_vue_generator.utils import vuetify, run, fail, cd_back, replace_in_file, overwrite
...
def prepare():
    with cd_back():
        if not run('which vue', True):
            if not run('which yarn', True):
                fail('which npm', silent=True, msg='Please install yarn or at least npm')
                fail('npm i -g yarn')
    ...
    with cd_back('frontend/'):
        ...
        replace_in_file('src/main.js', "import Vue from 'vue'",
                        '''import Vuelidate from 'vuelidate'\nVue.use(Vuelidate)''')
        replace_in_file('src/main.js', "import Vue from 'vue'",
                        '''import VueResource from 'vue-resource'\nVue.use(VueResource)''')
        replace_in_file('package.json', 'vue-cli-service build',
...
""")
    print("don't forget to add 'frontend' to INSTALLED_APPS and include frontend.urls to your urlpatterns")
